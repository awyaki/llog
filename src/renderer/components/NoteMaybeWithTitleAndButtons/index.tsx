...
import { TagsList, BlockList } from "~/components";
...
type INoteMaybeWithTitleAndButtons = (arg: {
  title?: string;
      ...
      <div css={isNested ? { padding: "5px" } : noteStyle} {...rest}>
        ...
        >
          <div>
            ...
            >
              {makeFormalTimeString(updatedAt)}
            </div>
          ...
          <ul css={{ display: "flex" }}>
            {Buttons?.map((Button, i) => (
              <li
                key={i}
                css={
                  Buttons.length - 1 !== i ? { marginRight: "8px" } : undefined
                }
              >
                {Button}
              </li>
