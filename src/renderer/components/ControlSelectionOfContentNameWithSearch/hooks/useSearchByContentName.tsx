...
> = (state, action) => {
  switch (action.type) {
    case "SEARCH_CONTENT_NAME/CALCULATE_WITH_NEW_CONTENT_NAMES": {
      ...
      const nextTokenMapOfContentName = createNGramTokenMap(
        action.contentNames.map(({ id, contentName }) => ({
          id,
          text: contentName,
        })),
      );
    ...
    case "SEARCH_CONTENT_NAME/SET_SEARCH_CONTENT_NAME_QUERY": {
      ...
      const nextFilteredContentNames = (() => {
        ...
        const idSetOfContentName = state.tokenMapOfContentNames.get(
          action.searchContentNameQuery,
        );
        if (idSetOfContentName === undefined) return [];
        return state.contentNames.filter(({ id }) =>
          idSetOfContentName.has(id),
        );
      })();
...
const init = (contentNames: ContentNameWithId[]): SearchByContentNameState => {
  ...
  const initialTokenMapOfContentNames = createNGramTokenMap(
    contentNames.map(({ id, contentName }) => ({ id, text: contentName })),
  );
...
export const useSearchByContentName = ({
  ...
  const [{ filteredContentNames, searchContentNameQuery }, dispatch] =
    useReducer(searchByContentNameRreducer, contentNames, init);
