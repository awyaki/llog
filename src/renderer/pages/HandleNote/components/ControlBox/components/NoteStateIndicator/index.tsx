...
export const NoteStateIndicator: VFC<Props> = ({
  ...
  const noteSavedState = (() => {
    if (!isNoteExist && isNoteChange)
      return { label: "warning", desc: "Not saved" };
    if (!isNoteExist && !isNoteChange)
      return { label: "normal", desc: "New note" };
    if (isNoteExist && isNoteChange)
      return { label: "warning", desc: "Not saved" };
    if (isNoteExist && !isNoteChange)
      return { label: "ok", desc: "No changes" };
    return { label: "", desc: "" };
  })();
  ...
  return (
    <div {...rest}>
      ...
      <div css={{ fontSize: font.size.S }}>
        {updatedAt ? makeFormalTimeString(updatedAt) : "No data"}
      </div>
    </div>
  );
};
