...
export const useEditNote = (content: ContentWithRelation) => {
  ...
  const { selectedBlocks, dispatch: selectedBlocksDispatch } = useContext(
    SelectedBlocksForHandleNoteContext,
  );

  const [mode, setMode] = useState<Mode>("edit");
  const { setIsAllowTransition, setConfirmerMessage } = useContext(
    IsAllowTransitionContext,
  );
  ...
  const isNoteChange = useMemo(() => {
    ...
    const isSelectedBlocksChange = !arrayeEqualWithId(
      note.blocks,
      selectedBlocks,
    );
  ...
  useEffect(() => {
    if (isNoteChange) {
      ...
      setConfirmerMessage(
        "If you leave this page now, you will lose all changes which are not saved.",
      );
    } else {
  ...
  const handleLink = useCallback(
    (path: string, isNoteChange: boolean) => () => {
      if (isNoteChange) {
        if (confirmer()) {
          history.push(path);
          return;
