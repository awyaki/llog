...
export const useUpdateContent = ({ content }: Props) => {
  ...
  const isMoreThanEqaulToPreviousNumber = useCallback<Validate<string>>(
    (numberOfBlocks) => {
      return (
        Number(numberOfBlocks) >= content.blocks.length ||
        `You should update the number to more than the previous one, ${content.blocks.length} or remain.`
      );
    },
    [content],
  );
