...
export const useSearchLogs = (initialLogs: LogWithRelation[]) => {
  ...
  const [contentNames, setContentNames] = useState<ContentNameWithId[] | null>(
    null,
  );
  ...
  const setContentNameQuery = useCallback(
  ...
  const toggleContentNameQuery = useCallback(
